...
char address[MAX_VAL] = "root/test.txt", word_find[MAX_VAL] = {}, fkadd[MAX_VAL] = {};
int f_save = 1;
char mode = 'N';
...
void re_files(char path[])
{
    ...
    for (ll i = 1 ; i < strlen(path) ; ++i){
        ...
        if (i == strlen(path)){
            FILE *fp = NULL;
            fp = fopen(dir, "r");
            if (fp != NULL) {fprintf(fout, "%s", "file is already existing\n"); {save_file(address);f_save = 1;strcpy(address, ".output.txt"); starting_line = 1;}fclose(fp); return;}
            else{
                fp = fopen(dir, "a");
                {strcpy(address, dir); starting_line = 1;}
                fclose(fp);
                return;
            }
        }
...
void re_cat(char *filename)
{
    ...
    FILE *fp;
    fp = fopen(filename, "r");
    if (fp == NULL){fprintf(fout, "%s", "file not found\n"); {save_file(address);f_save = 1;strcpy(address, ".output.txt"); starting_line = 1;}return;}
    int c;
    while(1)
    {
        c = fgetc(fp);
    ...
    if (fall == 0 && fat == 0)
    {
        ...
        for (int i = 0 ; i <= strlen(string) - strlen(message) ; i++)
        {
            ...
            if (flag == 1){
                flag2 = 1;
                int y, x;
                find_line(filename, i, &y, &x);
                if ((ry + starting_line < y + 1) || (y + 1== ry + starting_line && rx -3 <= x))
                {
                    if (flag_change == 0)
                    {
                        takey = y - starting_line + 1;
                        takex = x + 3;
                        flag_change = 1;
                    }
                }
        ...
        if (flag_change == 0) {takey = ry; takex = rx;}
        return;
    }
    if (fall == 1)
    {
...
void re_arman(char *input, char *word, int *counter_input, int *counter_word)
{
    ...
    else if (strcmp(command , "insertstr") == 0)
    {
        ...
        {strcpy(address, filename); starting_line = 1;}
    }
    else if (strcmp(command, "find") == 0)
    {
        ...
        {save_file(address);f_save = 1;strcpy(address, ".output.txt"); starting_line = 1;}
    }
    else if (strcmp(command, "grep") == 0)
    {
        ...
        if (counter_filenames == 0) {fprintf(fout, "Invalid input");return;}
...
{
    if (fkadd[0] == '/'){
        for (ll i = 1 ; i < strlen(fkadd) ; i++){
            fkadd[i - 1] = fkadd[i];
        }
        fkadd[strlen(fkadd) - 1] = '\0';
    }
    FILE* fp;
    fp = fopen(fkadd, "r");
    if (fp == NULL) {fprintf(fout, "file not found\n"); {save_file(address);f_save = 1;strcpy(address, ".output.txt"); starting_line = 1;} return;}
...
void find_phase2(WINDOW* win, char *filename, char *str, int f_save)
{
    init_pair(1, COLOR_BLACK, COLOR_GREEN);
    if (fkadd[0] == '/'){
        for (ll i = 1 ; i < strlen(fkadd) ; i++){
    ...
    if (fp == NULL) return;
    char c;
    int line_num = starting_line, pos_in_line = 0;
    int counter = 1, counter_highlight = 0, flag_coloring = 0, counter_myfile = 0, flag_select = 0;
    char myfile[MAX_VAL] = {};
    while(1)
    {
        c = fgetc(fp);
        if (feof(fp)) break;
        myfile[counter_myfile++] = c;
    }
    counter_myfile = 0;
    fclose(fp);
    fp = fopen(filename, "r");
    ...
    while (line_num < LINES - 4 + starting_line)
    {
        c = fgetc(fp);
        if (feof(fp)) break;
        if (counter_myfile + strlen(str) <= strlen(myfile))
        {
            int flagg = 0;
            for (int j = 0 ; j < strlen(str) ; j++)
            {
                if (str[j] != myfile[counter_myfile + j]) flagg = 1;
            }
            if (flagg == 0)
            {
                counter_highlight = strlen(str);
                flag_coloring = 1;
                if ((line_num > ry + starting_line) || (line_num == ry + starting_line && pos_in_line >= rx - 3))
                {
                    if (flag_select == 0)
                    {
                        takey = line_num - starting_line;
                        takex = pos_in_line + 3;
                        flag_select = 1;
                    }
                }
        ...
        if (counter_highlight > 0)
        {
            wattron(win, COLOR_PAIR(1));
            wrefresh(win);
            counter_highlight--;
        }
        else if (counter_highlight == 0)
        {
    ...
    fclose(fp);
    if (flag_coloring == 1) {wattroff(win, COLOR_PAIR(1)); wrefresh(win); flag_coloring = 0;}
    if (flag_select == 0) {takey = ry; takex = rx;}
    attron(A_STANDOUT);
    mvwprintw(win, LINES - 2, 0, "NORMAL | %s", filename);
    if (f_save == 0) wprintw(win, " +");
    attroff(A_STANDOUT);
...
int main()
{
    ...
    while (1)
    {
        ...
        if (mode == 'N')
        {
            ...
            while (1) // checking keyboard inputs
            {
                ...
                else if (c == '/')
                {
                    ...
                    find_phase2(win, address, word_find, f_save);
                    wmove(win, ry, rx);
                }
                else if (c == 'n')
                {
                ...
                else if (c == 'u') {re_undo_file(address); make_dis(win, f_save, mode, address); file_check(address); wmove(win, 0, 3); wrefresh(win);}
                else if (c == '=')
                {
            ...
            {
                if (counter_input == strlen(input)) strcpy(word, fkadd);
            ...
            else if (strcmp(command, "grep") == 0)
            {
                ...
                if (strlen(message) == 0 || counter_filenames == 0) {fprintf(fout, "Invalid input");{save_file(address);f_save = 1;strcpy(address, ".output.txt"); starting_line = 1;}continue;}
        ...
        if (mode == 'I')
        {
            ...
            while (1) // checking keyboard input
            {
                ...
                else if (c == KEY_BACKSPACE)
                {
                    ...
                    {
                        re_removestr(fkadd, cy + starting_line, cx - 4, 1, "-f");
                        make_dis(win, f_save, mode, address);
                ...
                else
                {
                    ...
                    char mess[10] = {};
                    mess[0] = (char) c;
                    re_insertstr(fkadd, mess, cy + starting_line, cx - 3);
                    make_dis(win, f_save, mode, address);
                    file_check(address);
                    if (c == '\n')
                    {
        ...
        if (mode == 'V')
        {
            ...
            wrefresh(win);
            getyx(win, cay, cax);
            make_dis_vis(win, f_save, address, ry, rx, ry, rx);
            wmove(win, ry, rx);
            int c;
            while (1) // checking keyboard input
            {
                ...
                else if (c == KEY_DOWN)
                {
                    ...
                    cx = MIN(cx, ends[cy + starting_line - 1] + 3);
                    make_dis_vis(win, f_save, address, ry, rx, cy, cx);
                    wmove(win, cy, cx);
                    wrefresh(win);
                }
                else if (c == KEY_UP)
                ...
                else if (c == 'y')
                {
                    ...
                    {
                        re_copy(fkadd, ry + starting_line , rx - 3, find_size(ry + starting_line - 1, rx - 3, cy + starting_line - 1, cx - 3) + 1, "-b");
                    }
                    else
                    {
                        re_copy(fkadd, ry + starting_line , rx - 3, find_size(ry + starting_line - 1, rx - 3, cy + starting_line - 1, cx - 3) + 1, "-f");
                    }
                    mode = 'N';
                    break;
                }
                else if (c == 'd')
                {
                    ...
                    if (ry > cy || (ry == cy && cx < rx))
                    {
                        re_cut(fkadd, ry + starting_line , rx - 3, find_size(ry + starting_line - 1, rx - 3, cy + starting_line - 1, cx - 3) + 1, "-b");
                    }
                    else
                    {
                        re_cut(fkadd, ry + starting_line , rx - 3, find_size(ry + starting_line - 1, rx - 3, cy + starting_line - 1, cx - 3) + 1, "-f");
                    }
                    make_dis_vis(win, f_save, address, ry, rx, cy, cx);
                    wrefresh(win);
                    file_check(address);
